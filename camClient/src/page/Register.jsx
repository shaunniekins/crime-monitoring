...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!credentials.first_name)
      return showErrorMessage("First Name is required");
    if (!credentials.last_name)
      return showErrorMessage("Last Name is required");
    ...
    if (credentials.password !== credentials.confirm_password)
      return showErrorMessage("Password do not match.");
    if (!/^\d{11}$/.test(credentials.phone_number))
      return showErrorMessage("Please enter a valid 11-digit phone number.");
    // INSERT
    await axios
      ...
      {step === 1 ? (
        <div className="w-full flex flex-col p-4 bg-white rounded-md gap-2 sm:w-4/6 md:w-3/6 md:rounded-tr-md md:rounded-br-md md:rounded-tl-none md:rounded-bl-none">
          ...
          <div className="flex relative gap-1 justify-center items-center rounded-md">
            ...
            <input
              type="checkbox"
              className="absolute top-4 right-5 md:right-6 lg:right-8 cursor-pointer"
